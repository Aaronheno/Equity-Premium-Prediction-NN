"""
grid_oos_7.py - FRED Variables Grid Search Out-of-Sample Evaluation
...
🔍 OPTIMIZATION: Grid Search hyperparameter optimization from in-sample results
...
using grid search optimization with comprehensive economic indicators.
"""
...
def run(
    ...
    """
    Run out-of-sample evaluation for grid search optimized models with FRED variables.
    ...
    # Find most recent in-sample run that contains "grid"
    grid_dirs = [d for d in in_sample_dir.iterdir() if d.is_dir() and "grid" in d.name.lower()]
    if not grid_dirs:
        raise FileNotFoundError(f"No grid search results found in {in_sample_dir}")
    
    latest_grid_dir = max(grid_dirs, key=lambda x: x.stat().st_mtime)
    print(f"Using in-sample results from: {latest_grid_dir}")
    
    # Set up output directory
    out_base = Path(f"./runs/7_FRED_Variables_OOS")
    out_base.mkdir(parents=True, exist_ok=True)
    run_name = f"{ts}_grid_oos"
    ...
    for model_name in models:
        ...
        best_params_file = latest_grid_dir / "studies_or_best_params" / f"{model_name}_best_params.joblib"
    ...
    with open(out_dir / "run_config.txt", "w") as f:
        ...
        f.write(f"In-sample directory: {latest_grid_dir}\n")
