"""
Bayesian Out-of-Sample Evaluation with MAE-Optimized Parameters

This experiment conducts out-of-sample evaluation using hyperparameters optimized via
Bayesian optimization with Mean Absolute Error (MAE) scoring. Features similar
parallelization potential as standard OOS evaluation with MAE-optimized parameters.
...
Performance Notes:
    - MAE-optimized parameters: May exhibit different OOS behavior than MSE-optimized
    ...
    - Alternative optimization: MAE parameters may provide better robust performance
...
def run(models=["Net1"], trials=10, epochs=30, threads=4, batch=256, 
        device='cpu', gamma_cer=3, oos_start_date=199001):
    """
    Run out-of-sample evaluation using Bayesian optimization hyperparameters optimized with MAE.
    ...
    trials : int
        Number of trials used in in-sample optimization
    epochs : int
    ...
    """
    print(f"Starting out-of-sample evaluation with MAE-optimized Bayesian optimization parameters")
    ...
    output_dir = Path(f"./runs/5_MAE_OOS_bayes_{timestamp}")
    ...
    with open(output_dir / "config.txt", "w") as f:
        f.write(f"Out-of-Sample Evaluation with MAE-Optimized Bayesian Optimization\n")
        f.write(f"=======================\n")
    ...
    # Find most recent in-sample MAE Bayesian optimization results
    is_results_dirs = sorted([d for d in Path("./runs").glob("5_MAE_IS_bayes_*")], reverse=True)
    
    if not is_results_dirs:
        print(f"Error: No in-sample MAE Bayesian optimization results found. Run bayes_mae_is_5.py first.")
        return
    ...
    for model_name in models:
        ...
        summary_data.append({
            "Model": model_name,
            "Method": "Bayesian Optimization (MAE)",
            "OOS R² (%)": oos_r2 * 100,
