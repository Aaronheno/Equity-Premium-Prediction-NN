...
def run_rolling_window(
    model_names=["Net1"],
    window_sizes=[5, 10, 20],  # Window sizes in years
    ...
    """
    Run rolling window analysis with hyperparameter optimization.
    ...
    window_sizes : list
        List of window sizes in years to evaluate
    ...
    print(f"Starting rolling window analysis for {model_names} with {optimization_method} optimization")
    print(f"Window sizes: {window_sizes} years")
    ...
    base_output_dir = Path(f"./runs/3_Rolling_Window_{optimization_method}_{timestamp}")
    
    if save_results:
        base_output_dir.mkdir(parents=True, exist_ok=True)
        # Save configuration
        with open(base_output_dir / "config.txt", "w") as f:
            f.write(f"Rolling Window Analysis\n")
            f.write(f"======================\n")
            f.write(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Models: {model_names}\n")
            f.write(f"Optimization method: {optimization_method}\n")
            f.write(f"Window sizes: {window_sizes} years\n")
    ...
    for model_name in model_names:
        ...
        for window_size in window_sizes:
            ...
            # For valid rolling window, we need at least window_size data before OOS start
            min_required_idx = oos_start_idx - window_months
            if min_required_idx < 0:
                print(f"  Warning: Not enough data for {window_size} year window before OOS start")
                print(f"  Need at least {window_months} months before OOS start date")
                continue
            ...
            for t in range(oos_start_idx, len(predictor_raw)):
                ...
                window_start_idx = max(0, t - window_months)
            ...
            print(f"  Completed processing for {window_size} year window")
            ...
            model_results[window_size] = window_results
            
            # Save results for this window size
            if save_results:
                window_dir = base_output_dir / f"{model_name}_window{window_size}"
    ...
    if save_results:
        ...
        if summary_data:
            ...
            with pd.ExcelWriter(base_output_dir / "rolling_window_results.xlsx") as writer:
