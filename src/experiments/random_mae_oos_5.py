"""
Random Search Out-of-Sample Evaluation with MAE-Optimized Parameters

This experiment conducts out-of-sample evaluation using hyperparameters optimized via
random search with Mean Absolute Error (MAE) scoring. Features similar parallelization
...
Performance Notes:
    - MAE-optimized parameters: Random search may find robust parameter combinations
    ...
    - Alternative optimization: MAE random search provides diverse parameter exploration
...
def run(models=["Net1"], trials=10, epochs=30, threads=4, batch=256, 
    ...
    """
    Run out-of-sample evaluation using random search hyperparameters optimized with MAE.
    ...
    """
    print(f"Starting out-of-sample evaluation with MAE-optimized random search parameters")
    ...
    output_dir = Path(f"./runs/5_MAE_OOS_random_{timestamp}")
    ...
    with open(output_dir / "config.txt", "w") as f:
        f.write(f"Out-of-Sample Evaluation with MAE-Optimized Random Search\n")
        f.write(f"=======================\n")
    ...
    # Find most recent in-sample MAE random search results
    is_results_dirs = sorted([d for d in Path("./runs").glob("5_MAE_IS_random_*")], reverse=True)
    
    if not is_results_dirs:
        print(f"Error: No in-sample MAE random search results found. Run random_mae_is_5.py first.")
        return
